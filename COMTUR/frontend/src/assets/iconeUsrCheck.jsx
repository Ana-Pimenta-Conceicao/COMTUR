import React from "react";

function IconeUsrCheck() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="108" height="128">
            <rect width="256" height="256" fill="none" />
            <circle cx="128" cy="120" r="40" fill="none" stroke="currentColor"
                strokeLinecap="round" strokeLinejoin="round" strokeWidth="12" />
            <path d="M63.8,199.37a72,72,0,0,1,128.4,0" fill="none" stroke="currentColor"
                strokeLinecap="round" strokeLinejoin="round" strokeWidth="12" />
            <path d="M222.67,112A95.92,95.92,0,1,1,144,33.33" fill="none" stroke="currentColor"
                strokeLinecap="round" strokeLinejoin="round" strokeWidth="12" />
            <polyline points="184 56 200 72 232 40" fill="none" stroke="#FFD121"
                strokeLinecap="round" strokeLinejoin="round" strokeWidth="14" />
        </svg>
    );
}

export default IconeUsrCheck;
