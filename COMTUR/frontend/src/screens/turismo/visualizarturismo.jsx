...
export default function VisualizarTurismos() {
  ...
  const [atualizarOutrosTurismos, setAtualizarOutrosTurismos] = useState(false);
  ...
  useEffect(() => {
    ...
    const obterOutrosTurismos = async () => {
      try {
        ...
        setOutrosTurismos(response.data.filter(t => t.id !== parseInt(id)));
        setAtualizarOutrosTurismos(true);
      } catch (error) {
  ...
  useEffect(() => {
    if (atualizarOutrosTurismos) {
      const outrosTurismosOrdenados = [...outrosTurismos].sort((a, b) => b.id - a.id);
      const outrosTurismosFiltrados = outrosTurismosOrdenados.slice(0, 3);
      setOutrosTurismos(outrosTurismosFiltrados);
    }
  }, [atualizarOutrosTurismos, outrosTurismos]);
  ...
  const abrirGoogleMaps = (local) => {
    window.open(`https://www.google.com/maps/search/?api=1&query=${local}`, '_blank');
  };
          ...
          <div className="flex justify-center items-center">
            <div className="pt-5 pb-5 px-2 text-xs sm:pl-32 sm:pr-32 sm:min-w-[320px] lg:w-[90%] xl:w-[80%] 2xl:w-[70%]">
                  ...
                  <div className="grid grid-cols-2 h-[140px] sm:h-[300px] border-2 border-[#DBDBDB]">
                    {outroTurismo.imagemTurismo[0] && (
                      <img
                        ...
                        alt="Preview"
                        className="w-full h-[136px] sm:h-[290px] object-cover"
                      />
                    )}
                    <div className="pl-6 pt-3">
                      ...
                      <div className="flex">
                        <button
                          ...
                          onClick={() => navigate(`/visualizarTurismo/${outroTurismo.id}`)}
                        >
                          Leia Mais
                        </button>
                      </div>
              ...
              <div className="flex items-center justify-center">
                <BtnAcao funcao={() => navigate('/todosTurismos')} acao="VisualizarMais" />
              </div>
            </div>
          </div>
        </>
