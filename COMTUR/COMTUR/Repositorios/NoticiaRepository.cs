    ...
    namespace COMTUR.Repositorios
    {
        ...
        public async Task<NoticiaModel> Adicionar(NoticiaModel noticiaModel)
        {
            ...
            if (noticiaModel.ImagemNoticia != null && noticiaModel.ImagemNoticia.Any())
            {
                foreach (var imagem in noticiaModel.ImagemNoticia)
                {
                    imagem.IdNoticia = noticiaModel.Id;
                }

                await _dbContext.ImagemNoticia.AddRangeAsync(noticiaModel.ImagemNoticia);
                await _dbContext.SaveChangesAsync();
            }
